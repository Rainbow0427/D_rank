...
class Calib:
    ...
    @staticmethod
    def get_hessian_approximation(group, name, model_type, calib_path=None):

        if name in ["self_attn.q_proj", "self_attn.v_proj"]:
            calib_name_to_use = "self_attn.k_proj"
        elif name == 'mlp.gate_proj':
            calib_name_to_use = 'mlp.up_proj'
        else:
            calib_name_to_use = name

        hessian_approx = Calib.get_calib_data(group, calib_name_to_use, calib_path)
        return hessian_approx.double()
