...
class Group:
    ...
    def _init_basis_coefficient(self, std_model, s, invs):
        if self.model_type == 'gpt2':
            ...
            w_slices = self._init_llama2(std_model)
        elif self.model_type == "opt":
            w_slices = self._init_opt(std_model)
        elif self.model_type == "mistral":
            w_slices = self._init_mistral(std_model)
        else:
    ...
    def change_basis(self, model, num_basis, basis_name):
        if self.model_type == 'gpt2':
            basis_parent_module = model.transformer
        elif self.model_type in ["llama2", "mistral"]:
            basis_parent_module = model.model
        elif self.model_type == "opt":
            basis_parent_module = model.model.decoder
        else:
            raise NotImplementedError

        basis_module_dict = getattr(basis_parent_module, basis_name)
        group_leader_idx = str(self.member[0])
        basis_module_dict[group_leader_idx].weight.data = self.basis[:, :num_basis].T.contiguous()
        return model
    ...
    def change_coefficient(self, model, num_basis):
        ...
        for i, layer in enumerate(self.member):
            ...
            tmp_model[layer].get_submodule(self.name).weight.data = weight.T.contiguous()
        return model
...
def update_model(std_model, model, model_type, groups, name, step, num_basis, basis_name, calib_path):
    if model_type == "gpt2":
        ...
        tmp_model = model.transformer.h
        tmp = model.transformer
    elif model_type in ["llama2", "mistral"]:
    ...
    for group in tqdm(groups):
        ...
        if basis_name in ["q_basis", "v_basis"]:
        ...
        for i, layer_idx in enumerate(group):
            ...
            tmp_model[layer_idx].get_submodule(name).weight.data = data.T.contiguous()
    return model
